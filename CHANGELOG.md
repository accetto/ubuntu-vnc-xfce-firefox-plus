...
- Firefox **77.0.1**
  - **IMPORTANT**: multi-threaded mode is default now, but larger shared memory is required (see bellow)
    ...
    - FIXED: [Issue #7 (Firefox 77.0.1 scrambles pages)](https://github.com/accetto/ubuntu-vnc-xfce-firefox-plus/issues/7)
    - Internet browsing should be sand-boxed now
    - tag `singleprocess` is introduced
      - which is single-threaded, but
      - please be aware that in **this** release (Firefox 77.0.1) webpages will be still scrambled (issue #7)
- Other changes:
